...
def objective_bndm(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        logger.info(
            f"BNDM Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"BNDM Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
...
def objective_csddm(trial: optuna.Trial) -> float:
    ...
    params = {
        'n_samples': trial.suggest_int('n_samples', 50, 500),
        'feature_proportion': trial.suggest_float('feature_proportion', 0.1,
                                                  1.0),
    ...
    try:
        ...
        accuracy, runtime, n_drifts = run_detector(detector, classifier_path)
        logger.info(
            f"CSDDM Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"CSDDM Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
        trial.set_user_attr('drifts', -1)
        return 0.0, float('inf')
    except Exception as e:
...
def objective_d3(trial: optuna.Trial) -> float:
    ...
    recent_samples_size = trial.suggest_int('recent_samples_size', 50, 5000)
    params = {
        'n_reference_samples': trial.suggest_int('n_reference_samples', 50,
                                                 5000),
        'recent_samples_proportion': trial.suggest_float(
    ...
    try:
        ...
        accuracy, runtime, n_drifts = run_detector(detector, classifier_path)
        logger.info(
            f"D3 Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"D3 Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
        trial.set_user_attr('drifts', -1)
        return 0.0, float('inf')
    except Exception as e:
...
def objective_ibdd(trial: optuna.Trial) -> float:
    ...
    try:
        detector = IBDD(
            n_samples=401,
            n_consecutive_deviations=6,
            n_permutations=513,
            update_interval=39,
            seed=SEED,
            recent_samples_size=1347,
        )
        ...
        accuracy, runtime, n_drifts = run_detector(detector, classifier_path)
        logger.info(
            f"IBDD Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"IBDD Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
        trial.set_user_attr('drifts', -1)
        return 0.0, float('inf')
    except Exception as e:
...
def objective_ocdd(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        accuracy, runtime, n_drifts = run_detector(detector, classifier_path)
        logger.info(
            f"OCDD Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"OCDD Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
        trial.set_user_attr('drifts', -1)
        return 0.0, float('inf')
    except Exception as e:
...
def objective_spll(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        accuracy, runtime, n_drifts = run_detector(detector, classifier_path)
        logger.info(
            f"SPLL Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"SPLL Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
        trial.set_user_attr('drifts', -1)
        return 0.0, float('inf')
    except Exception as e:
...
def objective_udetect(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        accuracy, runtime, n_drifts = run_detector(detector, classifier_path)
        logger.info(
            f"UDetect Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={n_drifts}")
        trial.set_user_attr('drifts', n_drifts)
        return accuracy, runtime

    except TimeoutError:
        logger.warning(
            f"UDetect Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
        trial.set_user_attr('drifts', -1)
        return 0.0, float('inf')
    except Exception as e:
...
def load_existing_trials(csv_path, study, detector_name):
    ...
    meta_columns = {'trial_id', 'accuracy', 'runtime', 'drifts', 'detector',
                    'dataset'}
    n_loaded = 0
    n_successful = 0
    n_skipped = 0
...
DETECTORS = {
    'BNDM': {
        ...
        'params': ['recent_samples_size', 'n_samples', 'const', 'threshold',
                   'max_depth'],
    },
    'CSDDM': {
        'objective': objective_csddm,
        'params': ['recent_samples_size', 'n_samples', 'feature_proportion',
                   'n_clusters', 'confidence'],
    },
    'D3': {
        'objective': objective_d3,
        'params': ['recent_samples_size', 'n_reference_samples',
                   'recent_samples_proportion', 'threshold'],
    },
    'IBDD': {
        'objective': objective_ibdd,
        'params': ['recent_samples_size', 'n_samples',
                   'n_consecutive_deviations', 'n_permutations',
                   'update_interval'],
    },
    'OCDD': {
    ...
    'SPLL': {
        ...
        'params': ['recent_samples_size', 'n_samples', 'n_clusters',
                   'threshold'],
    },
    'UDetect': {
        'objective': objective_udetect,
        'params': ['recent_samples_size', 'n_windows', 'n_samples',
                   'disjoint_training_windows'],
    },
}
...
def main():
    ...
    parser.add_argument('--detectors', type=str, nargs='+',
                        default=['BNDM', 'CSDDM', 'D3', 'IBDD', 'OCDD', 'SPLL',
                                 'UDetect'],
    ...
    for detector_name in args.detectors:
        ...
        results_csv = os.path.join(args.output_dir,
                                   f'{detector_name}_{DATASET}.csv')
        n_existing, n_existing_successful = load_existing_trials(results_csv,
                                                                 study,
                                                                 detector_name)
        ...
        if n_existing > 0:
            logger.info(
                f"Loaded {n_existing} existing trials ({n_existing_successful} successful) "
                f"for {detector_name} from {results_csv}")

        if remaining_successful == 0:
            logger.info(
                f"{detector_name} already has {n_existing_successful}/{args.n_trials} "
                f"successful runs — skipping")
        else:
            logger.info(
                f"Need {remaining_successful} more successful runs for {detector_name} "
            ...
            _csv_header_written = os.path.exists(
                _results_csv) and os.path.getsize(_results_csv) > 0
            ...
            def trial_callback(study, trial, _det=detector_name,
                ...
                fieldnames = _csv_fieldnames if _csv_fieldnames else list(
                    row.keys())
            ...
            while _n_new_successful < remaining_successful:
                batch = min(BATCH_SIZE,
                            remaining_successful - _n_new_successful)
                study.optimize(
                ...
                logger.info(
                    f"{detector_name}: {n_existing_successful + _n_new_successful}/{args.n_trials} "
                    f"successful runs ({n_existing + _n_new_trials} total trials)")

            logger.info(
                f"{detector_name}: Finished — {_n_new_successful} new successful runs "
        ...
        for t in sorted(pareto_trials, key=lambda t: t.values[0],
                        reverse=True):
            print(
                f"  Trial {t.number}: accuracy={t.values[0]:.4f}, runtime={t.values[1]:.1f}s")
    ...
    for detector_name, result in results.items():
        ...
        print(
            f"{detector_name:<10} {len(pareto):<18} {best_acc:.4f}           {best_rt:.1f}s")
