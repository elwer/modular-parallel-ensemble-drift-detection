...
class PCACD(UnsupervisedDriftDetector):
    ...
    def __init__(self, window_size=100, ev_threshold=0.99, delta=0.01,
                 divergence_metric="kl", sample_period=0.05,
                 online_scaling=True, seed : int = None,
    ...
    def _validate_X(self, X):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            ary = np.array(ary)
            if len(ary.shape) <= 1:
                # only one sample should be passed, so coerce
                # column vectors (e.g. pd.Series) to rows
                ary = ary.reshape(1, -1)
            if self._input_col_dim is None:
                # This allows starting with a dataframe,
                # then later passing bare
    ...
    def update(self, X, y_true=None, y_pred=None):
        ...
        X, _, _ = self._validate_input(X, None, None)
        ...
        if self._build_reference_and_test:
            if self.drift_state is not None:
                ...
                self._test_window = pd.DataFrame()
                self.reset()
                self._drift_detection_monitor._reset()
            ...
            if len(self._test_window) == self.window_size:
                ...
                for i in range(self.num_pcs):
                    ...
                    if self.divergence_metric == "intersection":
                        ...
                        self._density_reference[
                            f"PC{i + 1}"] = self._build_histograms(
            ...
            if self.online_scaling is True:
                ...
                next_obs = pd.DataFrame(X)
            self._test_window = pd.concat(
                [self._test_window.iloc[1:, :], next_obs])
            ...
            if (((self.total_samples - 1) % self.step) == 0) and (
                ...
                # some river versions have a different version of PH test, then use this
                #in_drift, _ = self._drift_detection_monitor.update(
                #    change_score)
                self._drift_detection_monitor.update(change_score)
                in_drift = self._drift_detection_monitor.drift_detected
    ...
    def _build_kde(cls, sample):
        ...
        kde_object = KernelDensity(bandwidth=bandwidth,
                                   kernel="epanechnikov").fit(
            sample.values.reshape(-1, 1)
        )
        # score_samples gives log-likelihood for each point, true density
        # values should be > 0 so exponentiate
        density = np.exp(
    ...
    def _build_histograms(sample, bins, bin_range):
        ...
        density = np.histogram(sample, bins=bins, range=bin_range,
                               density=True)
        return {
    ...
    def _jensen_shannon_distance(cls, density_reference, density_test):
        ...
        """
        js = jensenshannon(density_reference["density"],
