...
for fold in $(seq 0 $((N_REPEATS * 5 - 1))); do
  ...
  for detector in "${DETECTORS[@]}"; do
    for mode in expert generalist; do
      ...
      export STREAM_LENGTH N_TRIALS_EXPERT N_TRIALS_DEPLOYMENT PER_TRIAL_TIMEOUT N_JOBS SEED
      sbatch --parsable --export=ALL "${ROOT_DIR}/split_pipeline/cv_job.sbatch"
    done
  done
done

echo "Submitted 70 independent CV jobs (5 folds x 7 DD types x 2 methods)."
