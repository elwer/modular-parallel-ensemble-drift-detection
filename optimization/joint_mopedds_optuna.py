"""
Full Joint MOPEDDS Ensemble Optimization using Optuna.

This script performs joint optimization of MOPEDDS ensembles by running
Optuna to simultaneously optimize:
- All ensemble members (detector types and hyperparameters for each slot)
- MOPEDDS global parameters (decision criteria, windows, etc.)

Unlike the greedy approach, this optimizes all ensemble members jointly
in a single Optuna study rather than step-by-step.
"""
...
def _run_one_stream(generator_name: str, drift_frequency: int, stream_length: int,
                    ...
                    global_config: GlobalConfig) -> Tuple:
    """Run MOPEDDS ensemble on a single synthetic stream."""
    ...
    # Build detectors list from slot specs
    detectors = []
    for i, spec in enumerate(slot_specs):
        detector_type = spec['type']
        params = spec['params']
        det = get_detector(detector_type, params, seed=detector_seed_base + i + 1000 * s_idx)
        detectors.append(det)
    
    # Initialize MOPEDDS
    mopedds = MOPEDDS()
    mopedds.detectors = detectors
    mopedds.detector_decision_criteria = global_config.detector_decision_criteria
    mopedds.ensemble_decision_criteria = global_config.ensemble_decision_criteria
    mopedds.decision_window = global_config.decision_window
    mopedds.suppression_window = global_config.suppression_window
    mopedds.recent_samples_size = global_config.recent_samples_size
...
def evaluate_ensemble(generators: List[str], drift_frequencies: List[int],
                     ...
                     slot_specs: List[Dict], global_config: GlobalConfig,
...
def hyperparameter_ranges(detector_type: str, trial: optuna.Trial) -> Dict:
    ...
    if detector_type == 'IBDD':
        return {
            'n_samples': trial.suggest_int('ibdd_n_samples', 500, 2000),
            'n_consecutive_deviations': trial.suggest_int('ibdd_n_consecutive_deviations', 3, 20),
            'n_permutations': trial.suggest_int('ibdd_n_permutations', 100, 1000),
            'update_interval': trial.suggest_int('ibdd_update_interval', 5, 100),
        }
    elif detector_type == 'OCDD':
        return {
            'n_samples': trial.suggest_int('ocdd_n_samples', 500, 2000),
            'threshold': trial.suggest_float('ocdd_threshold', 0.1, 0.9),
        }
    elif detector_type == 'D3':
        return {
            'n_reference_samples': trial.suggest_int('d3_n_reference_samples', 500, 2000),
            'recent_samples_proportion': trial.suggest_float('d3_recent_samples_proportion', 0.1, 0.9),
            'threshold': trial.suggest_float('d3_threshold', 0.1, 0.9),
        }
    elif detector_type == 'SPLL':
        return {
            'n_samples': trial.suggest_int('spll_n_samples', 500, 2000),
            'n_clusters': trial.suggest_int('spll_n_clusters', 2, 20),
            'threshold': trial.suggest_float('spll_threshold', 0.1, 10.0),
        }
    elif detector_type == 'UDetect':
        return {
            'n_windows': trial.suggest_int('udetect_n_windows', 3, 10),
            'n_samples': trial.suggest_int('udetect_n_samples', 50, 500),
            'disjoint_training_windows': trial.suggest_categorical('udetect_disjoint_training_windows', [True, False]),
        }
        ...
        return {
            'n_samples': trial.suggest_int('bndm_n_samples', 500, 2000),
            'const': trial.suggest_float('bndm_const', 0.1, 1.0),
            'threshold': trial.suggest_float('bndm_threshold', 0.1, 1.0),
            'max_depth': trial.suggest_int('bndm_max_depth', 5, 20),
        }
    else:
...
def create_optuna_objective(generators: List[str], drift_frequencies: List[int],
                           ...
                           detector_seed: int, ensemble_size: int) -> callable:
    """Create Optuna objective function."""
    
    def objective(trial: optuna.Trial) -> float:
        # Sample MOPEDDS global parameters
        detector_decision_criteria = trial.suggest_categorical('detector_decision_criteria', ['any', 'all', 'majority'])
        ensemble_decision_criteria = trial.suggest_categorical('ensemble_decision_criteria', ['any', 'all', 'majority'])
        decision_window = trial.suggest_int('decision_window', 1, 100)
        suppression_window = trial.suggest_int('suppression_window', 1, 100)
        ...
        # Sample detector types and hyperparameters for each slot
        slot_specs = []
        for i in range(ensemble_size):
            detector_type = trial.suggest_categorical(f'slot{i}_type', ['IBDD', 'OCDD', 'D3', 'SPLL', 'UDetect', 'CSDDM', 'BNDM'])
            params = hyperparameter_ranges(detector_type, trial)
            slot_specs.append({'type': detector_type, 'params': params})
...
def joint_optimize_mopedds(*,
    ...
    logger.info(f"Starting joint MOPEDDS optimization for ensemble size {ensemble_size}")
    ...
    study = optuna.create_study(
        study_name=f"joint_mopedds_size{ensemble_size}",
    ...
    slot_specs = []
    for i in range(ensemble_size):
        detector_type = best_trial.params[f'slot{i}_type']
        params = {k: v for k, v in best_trial.params.items() if k.startswith(detector_type.lower() + '_')}
        params = {k[len(detector_type.lower()) + 1:]: v for k, v in params.items()}
        slot_specs.append({'type': detector_type, 'params': params})
...
if __name__ == "__main__":
    ...
    with open(args.output_csv, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['best_trial_value', 'eval_f1', 'global_config', 'slot_specs'])
        writer.writeheader()
