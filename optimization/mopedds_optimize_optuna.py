...
def objective(trial: optuna.Trial):
    ...
    try:
        ...
        logger.info(
            f"Trial {trial.number}: accuracy={accuracy:.4f}, runtime={runtime:.1f}s, drifts={len(drifts)}")
        ...
        logger.warning(
            f"Trial {trial.number} timed out after {TRIAL_TIMEOUT}s — skipping")
...
def main():
    ...
    csv_header_written = os.path.exists(results_csv) and os.path.getsize(
        results_csv) > 0
    ...
    print(
        f"Existing trials: {n_existing} ({n_existing_successful} successful)")
    ...
    if remaining_successful == 0:
        logger.info(
            f"Already have {n_existing_successful}/{args.n_trials} successful runs — skipping optimization")
    else:
        if n_existing > 0:
            logger.info(
                f"Loaded {n_existing} existing trials ({n_existing_successful} successful) from {results_csv}")
        ...
        while _n_new_successful < remaining_successful:
            ...
            logger.info(
                f"MOPEDDS: {n_existing_successful + _n_new_successful}/{args.n_trials} "
                f"successful runs ({n_existing + _n_new_trials} total trials)")

        logger.info(
            f"MOPEDDS: Finished — {_n_new_successful} new successful runs "
    ...
    print("\n" + "=" * 80)
    print(
        f"Best accuracy configuration as mopedds.config YAML (Trial {best_trial.number}: accuracy={best_trial.values[0]:.4f}, runtime={best_trial.values[1]:.1f}s):")
    print("=" * 80)
    print(f"""
...
""")
    ...
    if fastest_trial.number != best_trial.number:
        ...
        print("\n" + "=" * 80)
        print(
            f"Fastest configuration as mopedds.config YAML (Trial {fastest_trial.number}: accuracy={fastest_trial.values[0]:.4f}, runtime={fastest_trial.values[1]:.1f}s):")
        print("=" * 80)
        print(f"""
