"""
Single MOPEDDS Member Optimization using Optuna.

This script performs optimization of a single MOPEDDS ensemble member (ensemble size 1)
using Optuna, with the same multistream evaluation setup as the joint MOPEDDS optimization.
This allows fair comparison between single MOPEDDS member performance and full ensemble
performance with the same computational budget (500 trials).
"""
...
def _run_one_stream(generator_name: str, drift_frequency: int, stream_length: int,
                    ...
                    detector_params: Dict, global_config: GlobalConfig,
                    detector_seed: int, s_idx: int) -> Tuple:
    """Run single MOPEDDS member on a synthetic stream."""
    ...
    # Initialize MOPEDDS with single detector
    mopedds = MOPEDDS()
    mopedds.detectors = [det]
...
def evaluate_mopedds_member(generators: List[str], drift_frequencies: List[int],
                             stream_length: int, stream_seeds: List[int],
                             tolerances: List[int], indices: List[int],
                             detector_type: str, detector_params: Dict,
                             global_config: GlobalConfig, detector_seed: int) -> Dict:
    """Evaluate single MOPEDDS member on given stream indices."""
...
def hyperparameter_ranges(detector_type: str, trial: optuna.Trial) -> Dict:
    ...
    if detector_type == 'IBDD':
        ...
        return {
            ...
            'n_clusters': trial.suggest_int('n_clusters', 2, 20),
            'confidence': trial.suggest_float('confidence', 0.01, 0.1),
        }
...
def create_optuna_objective(generators: List[str], drift_frequencies: List[int],
    ...
    """Create Optuna objective function for single MOPEDDS member."""
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        metrics = evaluate_mopedds_member(
...
def optimize_single_mopedds_member(*,
                                    optuna_storage: str,
                                    generators: List[str],
    ...
    """Single MOPEDDS member optimization using Optuna."""
    logger.info(f"Starting single MOPEDDS member optimization for {detector_type}")
    ...
    params = {k: v for k, v in best_trial.params.items() if not k.startswith('detector_decision_criteria') and not k.startswith('ensemble_decision_criteria') and not k.startswith('decision_window') and not k.startswith('suppression_window') and not k.startswith('recent_samples_size')}
    
    # Evaluate best configuration on eval set
    eval_metrics = evaluate_mopedds_member(
        generators=generators,
        drift_frequencies=drift_frequencies,
        stream_length=stream_length,
        stream_seeds=stream_seeds,
...
if __name__ == "__main__":
    ...
    result = optimize_single_mopedds_member(
    ...
    with open(args.output_csv, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['detector_type', 'best_trial_value', 'eval_f1', 'global_config', 'best_params'])
        writer.writeheader()
