...
class MD3(SemisupervisedDriftDetector):
    ...
    def calculate_margin_inclusion_signal(self, sample, clf):
        ...
        Args:
            ...
            clf (sklearn.svm.SVC): the classifier for which we are calculating margin
                inclusion signal.
        """
        if isinstance(clf, Classifiers):
            # use the internal clf instead of our wrapper class
            if not isinstance(clf, sklearn.svm.SVC):
                raise ValueError(
                    """margin density only implemented for SVMs. 
                    Please use a SVM or implement the margin density for your
                    classifier."""
                )
            w = np.array(clf.clf.coef_[0])
            intercept = np.array(clf.clf.intercept_)
        else:
    ...
    def __init__(self, clf=svm.SVC(kernel='linear'),
                 margin_calculation_function=calculate_margin_inclusion_signal,
                 sensitivity=1.5, k=10, oracle_data_length_required=1000):
        """
        Args:
        ...
        self.reference_batch_target = None
        self.reference_batch_features = None
        self.curr_margin_density = None
        self.classifier = clf
    ...
    def calculate_distribution_statistics(self, data):
        ...
        passed in:
            ...
            5. Standard Deviation of Accuracy (acc_std)
        ...
        duplicate_classifier = self.classifier.clone()
        ...
        """
        perform k-fold cross validation to acquire 
        distribution margin density and acuracy values
        """
        for train_index, test_index in cv.split(self.reference_batch_features):
            ...
            #y_train = self.reference_batch_target.iloc[
            #train_index].values.ravel()
            #y_test = self.reference_batch_target.iloc[
            #test_index].values.ravel()
            for idx, sample in X_train.iterrows():
                duplicate_classifier.fit(sample, y_train.loc[idx, 0])
    ...
    def update(self, X, y_true=None, y_pred=None):
        ...
        # TODO: check, first we transform to dataframe, then to numpy again?
        oracle_label = copy.deepcopy(X)
        oracle_label['target'] = self.labels[-1]

        if self.waiting_for_oracle:
            self.give_oracle_label(oracle_label)
        else:
            sample_np_array = np.array(list(X.values()))
            margin_inclusion_signal = self.margin_calculation_function(
            ...
            warning_threshold = self.sensitivity * self.reference_distribution[
                "md_std"]
    ...
    def give_oracle_label(self, labeled_sample):
        ...
        if not self.waiting_for_oracle:
            raise ValueError(
        ...
        if len(self.oracle_data) == self.oracle_data_length_required:
            ...
            drift_level = self.reference_distribution[
                              "acc"] - acc_labeled_samples
            drift_threshold = self.sensitivity * self.reference_distribution[
                "acc_std"]
            ...
            self.set_reference(self.oracle_data, target_name=-1)
            self.oracle_data = None
            self.waiting_for_oracle = False
    ...
    def run_stream(self, stream, n_training_samples: int, classifier_path):
        ...
        """
        scaling should be done consistently, across DDs
        scaler = MinMaxScaler()
        
        x_values = [sample[0] for sample in
                    self.recent_samples]  # List of feature dictionaries
        y_values = [sample[1] for sample in self.recent_samples]  # List of labels

        x_df = pd.DataFrame(x_values)
        y_df = pd.Series(y_values, name='target')

        scaler.fit(x_df)
        x_normalized = scaler.transform(x_df)
        x_normalized_df = pd.DataFrame(x_normalized)
        recent_samples_df = pd.concat([x_normalized_df, y_df], axis=1)
        self.classifier.fit(x_normalized_df, y_df.values.ravel())
        #######################
        """
        #TODO: add scaling part
        features = []
        labels = []
        for sample in training_data:
            features.append(list(sample[0].values()))
            labels.append(sample[1])
        training_data = pd.concat([pd.DataFrame(features),
                                   pd.DataFrame(labels)], axis=1)
        self.set_reference(training_data)
        ...
        for i, (x, y) in enumerate(islice(stream, n_training_samples, None),
            ...
            # md3 requests "oracle"/true labels after warning level
            # only in case of a subsequent detected drift, reset/re-train
            if self.update(x):  # Use the drift detector's update method

                self.drifts.append(i)

            if self.drift_state == "drift" and not self.waiting_for_oracle:
                self.reset()
