...
class Node:
    ...
    def update(self, X: ArrayLike, pred_baseline_diff: ArrayLike,
               id_generator: Any):
        """Updates the node.
        ...
        # ----------------------------
        self.age += 1
        self.X_window = self._update_window(window=self.X_window, data=X,
                                            window_size=self.max_node_size)
        self.centroid = np.mean(self.X_window, axis=0)
        self.pred_baseline_diff_window = self._update_window(
            window=self.pred_baseline_diff_window,
            data=pred_baseline_diff,
            window_size=self.max_node_size)

        if self.is_leaf:
            # Check where the minimum similarity from the centroid is violated.
            similarities = rbf_kernel(self.X_window,
                                      self.centroid.reshape(1, -1))
            violations = similarities < self.gamma

            if np.count_nonzero(
                    violations) > 0 and self.depth + 1 <= self.max_tree_depth:
                # ----------------------------
                # Split the leaf node.
                # ----------------------------
                self.is_leaf = False
                ...
                child_centroids = np.unravel_index(proximity_matrix.argmin(),
                                                   proximity_matrix.shape)
                clusters = self._cluster(X=self.X_window,
                                         left_centroid=self.X_window[
                                             child_centroids[0]],
                                         right_centroid=self.X_window[
                                             child_centroids[1]])
                ...
                for child_i in range(2):
                    self.children.append(Node(node_id=next(id_generator),
                                              ...
                                              depth=self.depth + 1))
                    self.children[
                        -1].age = self.age - 1  # Note that the age is incremented again in the following update() call.
                    self.children[-1].update(
                        X=self.X_window[clusters == child_i],
                        pred_baseline_diff=self.pred_baseline_diff_window[
                            clusters == child_i],
                ...
                # Update p-values.
                # ----------------------------
                if self.pred_baseline_diff_window.shape[
                    0] >= 4:  # We require at least 4 observations to provide two disjoint samples to the t-test.
                    self.p_value = self._update_p_value(
                        X=self.pred_baseline_diff_window)
            ...
            clusters = self._cluster(X=X,
                                     left_centroid=self.children[
                                         0].centroid.reshape(1, -1),
                                     right_centroid=self.children[
                                         1].centroid.reshape(1, -1))

            for child_i in np.unique(clusters):
                self.children[
                    child_i].age = self.age - 1  # Note that the age is incremented again in the following update() call.
    ...
    @staticmethod
    def _update_window(window: Optional[ArrayLike], data: ArrayLike,
                       window_size: int) -> ArrayLike:
    ...
    @staticmethod
    def _cluster(X: ArrayLike, left_centroid: ArrayLike,
                 right_centroid: ArrayLike) -> ArrayLike:
        ...
        sim = np.append(sim_left.reshape(-1, 1), sim_right.reshape(-1, 1),
                        axis=1)
...
class CDLEEDS(SemisupervisedDriftDetector):
    """CDLEEDS.
    ...
    Attributes:
        ...
        baseline_y (ArrayLike): The baseline value, specified/updated by the user via set_baseline().
    """
    ...
    def __init__(self,
                 ...
                 max_time_stationary: int = 20,
                 seed: int = None,
                 recent_samples_size: int = 200):
    ...
    def set_baseline(self, input_dict):
        ...
        """
        self.baseline_y = np.array(list(input_dict.values())).reshape(1, -1)
    ...
    def partial_fit(self, X: ArrayLike, y_pred: ArrayLike):
        ...
        if self.baseline_y is None:
            raise AttributeError(
                'No CDLEEDS baseline value has been specified. '
                'Use set_baseline() to set or update the baseline.')
        elif self.baseline_y.shape != y_pred.shape:
            raise ValueError(
                'CDLEEDS baseline {} and y_pred {} have different shapes but should have the same.'.format(
                    self.baseline_y.shape, y_pred.shape))
        else:
            # Start recursive update at the root node.
            self.root.update(X=X, pred_baseline_diff=y_pred - self.baseline_y,
    ...
    def detect_local_change(self) -> Tuple[ArrayLike, ArrayLike, ArrayLike]:
        ...
        if self.monitored_sample:
            ...
            sample = np.asarray(
                [s['x'] for s in self.monitored_sample.values()])
            old_neighborhoods = np.asarray(
                [s['neighborhood'] for s in self.monitored_sample.values()])
            ...
            update_keys = [list(self.monitored_sample.keys())[i] for i in
                           np.argwhere(
                               new_neighborhoods != old_neighborhoods).flatten()]
            if len(update_keys) > 0:
                update_values = [dict(zip(['x', 'neighborhood'], (x, n))) for
                                 x, n in zip(
                        sample[new_neighborhoods != old_neighborhoods],
                        new_neighborhoods[
                            new_neighborhoods != old_neighborhoods])]
                update_dict = dict(zip(update_keys, update_values))
                self.monitored_sample = {**self.monitored_sample,
                                         **update_dict}

            # Identify monitored observations that have been assigned to a new or temporally changing neighborhood.
            changed_neighbors = (new_neighborhoods != old_neighborhoods) & (
                    old_neighborhoods != None)
            changing_monitored_samples = sample[changed_neighbors].tolist()
            changing_monitored_samples.extend(
                sample[local_drifts[leaf_indices] & ~changed_neighbors])
    ...
    def detect_global_change(self) -> bool:
        ...
        for class_col in range(p_values.shape[1]):
            ...
            p_col[
                p_col == 0] = 1e-10  # if p-value at a leaf is 0 then set it to a very small positive number instead.

            if len(p_col) > 0:
                _, combined_p_value[class_col] = combine_pvalues(
                    p_col)  # Combine p-values with Fisher's method.
                # Compute mean FDR alpha (alpha adjusted for k independent tests)
                fdr_controlled_alpha[class_col] = self.significance * (
                        len(p_col) + 1) / (2 * len(p_col))
    ...
    def update(self, x):
        self.partial_fit(x[0], x[1])
        return self.detect_global_change()

    @computational_metrics
    def process_main_stream(self, stream, n_training_samples, clf):
        ...
        for i, (x, y) in enumerate(islice(stream, n_training_samples, None),
            ...
            x_np = np.array(list(x.values())).reshape(1, -1)
            y_prob_np = np.array(list(y_prob.values())).reshape(1, -1)

            # Check for local change.
            # local_drifts, centroids, monitored_obs_subject_to_change = (
            #    self.detect_local_change())
            # Check for global change.

            if self.update((x_np, y_prob_np)):
            ...
            # Update the baseline.
            for key in x:
                self.baseline_x[key] = 0.001 * x[key] + (
                        1 - 0.001) * self.baseline_x.get(key, 0)

            self.set_baseline(clf.predict_proba(self.baseline_x))
    ...
    def run_stream(self, stream, n_training_samples: int, classifier_path):
        ...
        training_data, clf = self.load_main_clf(stream, n_training_samples,
                                                classifier_path)

        # Initialize a dictionary to store the sum of all features
        sum_x = {}
        n_samples = 0
        x_init_list = []
        ...
        for x, y in training_data:
            n_samples += 1
            x_init_list.append(list(x.values()))
            # Accumulate feature values in sum_x
            for feature, value in x.items():
                if feature not in sum_x:
                    sum_x[feature] = 0
                sum_x[feature] += value

        # Now, calculate the mean for each feature
        self.baseline_x = {feature: total / n_samples for feature, total in
                           sum_x.items()}

        self.set_baseline(clf.predict_proba(self.baseline_x))

        x_init = np.array(x_init_list)
        self.add_to_monitored_sample(x_init)

        return self.process_main_stream(stream, n_training_samples, clf)
