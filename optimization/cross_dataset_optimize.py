...
ALL_DATASETS = ['Electricity', 'GasSensor', 'ForestCovertype', 'PokerHand',
...
def run_detector_on_dataset(detector, dataset_name, tmp_dir):
    ...
    drifts, labels, predictions, n_req_labels, runtime, peak_memory, mean_memory = \
        detector.run_stream(dataset, N_TRAINING_SAMPLES,
                            get_classifier_path(dataset_name))
...
def make_detector(detector_name, params):
    ...
    if detector_name == 'BNDM':
        ...
        return OCDD(n_samples=params['n_samples'],
                    threshold=params['threshold'], **common)
    elif detector_name == 'SPLL':
        return SPLL(n_samples=params['n_samples'],
                    n_clusters=params['n_clusters'],
                    threshold=params['threshold'], **common)
    elif detector_name == 'UDetect':
        return UDetect(n_windows=params['n_windows'],
                       n_samples=params['n_samples'],
                       disjoint_training_windows=params[
                           'disjoint_training_windows'], **common)
    else:
    ...
    config = {
        ...
        'detectors': [
            ...
            {'class': 'detectors.udetect.UDetect', 'params': {
                ...
                'disjoint_training_windows': params[
                    'udetect_disjoint_training_windows']}},
        ]
    }
...
def get_param_distributions(detector_name):
    ...
    if detector_name == 'BNDM':
        ...
        dists['confidence'] = CategoricalDistribution(
        ...
        dists['disjoint_training_windows'] = CategoricalDistribution(
            [True, False])
    elif detector_name == 'MOPEDDS':
        dists.update({
            'detector_decision_criteria': CategoricalDistribution(
                ['any', 'majority', 'all']),
            'ensemble_decision_criteria': CategoricalDistribution(
                ['any', 'majority', 'all']),
            ...
            'csddm_confidence': CategoricalDistribution(
                [0.25, 0.1, 0.05, 0.025, 0.01, 0.005, 0.001]),
            ...
            'udetect_disjoint_training_windows': CategoricalDistribution(
                [True, False]),
        })
    return dists
...
def _param_in_distribution(value, dist):
    ...
    if value is None:
        return False
    if isinstance(dist, IntDistribution):
        return isinstance(value, (int, float)) and dist.low <= int(
            value) <= dist.high
    if isinstance(dist, FloatDistribution):
        return isinstance(value, (int, float)) and dist.low <= float(
            value) <= dist.high
...
def load_existing_trials(trials_path, study, detector_name, fold_idx):
    ...
    with open(trials_path, 'r', newline='') as f:
        ...
        for row in reader:
            ...
            trial = FrozenTrial(
                ...
                values=[float(row['avg_accuracy']),
                        float(row['avg_norm_runtime'])],
                datetime_start=None,
    ...
    if n_skipped:
        logger.warning(
            f"Skipped {n_skipped} trial(s) with out-of-range parameters")
    return n_loaded
...
def suggest_single_dd_params(trial, detector_name):
    ...
    params = {
        'recent_samples_size': trial.suggest_int('recent_samples_size', 50,
                                                 5000)}
    ...
    if detector_name == 'BNDM':
        ...
        params['n_samples'] = trial.suggest_int('n_samples', 50, 500)
        params['feature_proportion'] = trial.suggest_float(
            'feature_proportion', 0.1, 1.0)
        ...
        params['n_reference_samples'] = trial.suggest_int(
            'n_reference_samples', 50, 5000)
        params['recent_samples_proportion'] = trial.suggest_float(
            'recent_samples_proportion', 0.05, 0.5)
        params['threshold'] = trial.suggest_float('threshold', 0.1, 0.9)
    elif detector_name == 'IBDD':
        params['n_samples'] = trial.suggest_int('n_samples', 100, 2000)
        params['n_consecutive_deviations'] = trial.suggest_int(
            'n_consecutive_deviations', 1, 20)
        params['n_permutations'] = trial.suggest_int('n_permutations', 100,
                                                     1000)
        params['update_interval'] = trial.suggest_int('update_interval', 10,
                                                      100)
    elif detector_name == 'OCDD':
        params['n_samples'] = trial.suggest_int('n_samples', 50, 500)
        params['threshold'] = trial.suggest_float('threshold', 0.1, 0.9)
...
def suggest_mopedds_params(trial):
    ...
    params = {
        ...
        'csddm_feature_proportion': trial.suggest_float(
            'csddm_feature_proportion', 0.1, 1.0),
        ...
        'd3_n_reference_samples': trial.suggest_int('d3_n_reference_samples',
                                                    50, 500),
        'd3_recent_samples_proportion': trial.suggest_float(
            'd3_recent_samples_proportion', 0.05, 0.5),
        'd3_threshold': trial.suggest_float('d3_threshold', 0.1, 0.9),
        'ibdd_n_samples': trial.suggest_int('ibdd_n_samples', 100, 2000),
        'ibdd_n_consecutive_deviations': trial.suggest_int(
            'ibdd_n_consecutive_deviations', 1, 20),
        'ibdd_n_permutations': trial.suggest_int('ibdd_n_permutations', 100,
                                                 1000),
        'ibdd_update_interval': trial.suggest_int('ibdd_update_interval', 10,
                                                  100),
...
def make_objective(detector_name, train_datasets, tmp_dir):
    ...
    def objective(trial):
        ...
        _, avg_acc, avg_norm_rt = evaluate_on_datasets(detector_name, params,
                                                       train_datasets, tmp_dir)
...
def select_pareto_configs(pareto_trials, n=10):
    ...
    if len(selected) < n:
        ...
        extra_indices = np.linspace(0, len(remaining) - 1, n - len(selected),
                                    dtype=int)
...
def run_for_detector(detector_name, n_trials, output_dir, fold_idx=None):
    ...
    trials_path = os.path.join(output_dir,
                               f'{detector_name}_optimization_trials{fold_suffix}.csv')
    # If the file already exists (with content), we must append, not overwrite
    trials_header_written = os.path.exists(trials_path) and os.path.getsize(
        trials_path) > 0
    ...
    eval_path = os.path.join(output_dir,
                             f'{detector_name}_pareto_evaluation{fold_suffix}.csv')
    eval_header_written = os.path.exists(eval_path) and os.path.getsize(
        eval_path) > 0
    ...
    for fi, test_datasets in folds_to_run:
        ...
        n_existing = load_existing_trials(trials_path, study, detector_name,
                                          fi)
        remaining_trials = max(0, n_trials - n_existing)

        if n_existing > 0:
            logger.info(
                f"  Loaded {n_existing} existing trials for fold {fi} from {trials_path}")

        if remaining_trials == 0:
            logger.info(
                f"  Fold {fi} already has {n_existing}/{n_trials} trials — skipping optimization")
        else:
            logger.info(
                f"  Running {remaining_trials} new trials (existing: {n_existing}, target: {n_trials})")

            # Callback to write each completed trial to CSV immediately
            def trial_callback(study, trial, _fi=fi, _train=train_datasets,
                               _test=test_datasets):
                ...
                fieldnames = trials_fieldnames if trials_fieldnames else list(
                    row.keys())
            ...
            logger.info(
                f"Wrote {n_trials_written} new trial rows so far to {trials_path}")
        ...
        if existing_eval_rows > 0:
            logger.info(
                f"  Fold {fi} already has {existing_eval_rows} evaluation rows — skipping Pareto evaluation")
            continue

        completed = [t for t in study.trials if
                     t.state == optuna.trial.TrialState.COMPLETE]
        pareto = compute_pareto_front(completed)
        selected = select_pareto_configs(pareto, n=10)

        logger.info(
            f"  Pareto front: {len(pareto)} trials, selected {len(selected)} configs")

        # ── Evaluate selected configs on holdout datasets ─────────────────
        for sel_idx, trial in enumerate(selected):
            params = trial.params
            logger.info(
                f"  Evaluating config {sel_idx} (trial {trial.number}) on {test_datasets}")
            ...
            for ds in test_datasets:
                ...
                fieldnames = eval_fieldnames if eval_fieldnames else list(
                    eval_row.keys())
...
def main():
    parser = ArgumentParser(
        description='Cross-dataset hyperparameter optimization for drift detectors.')
    parser.add_argument('--detector', type=str, required=True,
                        choices=['BNDM', 'CSDDM', 'D3', 'IBDD', 'OCDD', 'SPLL',
                                 'UDetect', 'MOPEDDS'],
    ...
    parser.add_argument('--output_dir', type=str,
                        default='optimization/cross_dataset_results',
    ...
    run_for_detector(args.detector, args.n_trials, args.output_dir,
                     fold_idx=args.fold)
